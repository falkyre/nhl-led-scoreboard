    ...
    def wxDrawAlerts(self):
        ...
        while True:
            ...
            if self.data.wx_alerts[1] == "warning":
                if self.data.config.wxalert_alert_feed.lower() == "nws":
                    self.matrix.draw.rectangle(
                        [0, 0, self.matrix.width, self.banner_height], fill=self.data.wx_alerts[5]
                    )  # warning
                    self.matrix.draw.rectangle(
                        [0, self.matrix.height - self.banner_height, self.matrix.width, self.matrix.height],
                        fill=self.data.wx_alerts[5],
                    )  # warning
                else:
                    self.matrix.draw.rectangle(
            ...
            if self.scroll:
                ...
                self.matrix.draw_text_layout(self.layout4.warning, self.data.wx_alerts[0])
                self.matrix.draw_text_layout(self.layout4.warning_date, self.data.wx_alerts[2])
                self.matrix.render()
                sleep(1)
                i += 1
                if i == self.duration:
